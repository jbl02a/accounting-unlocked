  ...
  return (
    <div className={`flex items-center py-2 px-3 rounded-lg ${isCorrect ? 'bg-green-900/30' : isWrong ? 'bg-red-900/30' : 'bg-slate-800/50'}`}>
      ...
      <div className="text-right">
        <span className={`text-sm font-mono ${side === 'debit' ? 'text-white' : 'text-slate-200'}`}>
          {side === 'debit' ? `$${amount}` : ''}
        </span>
        <span className="w-16 inline-block text-right text-sm font-mono text-slate-200">
          {side === 'credit' ? `$${amount}` : ''}
        </span>
      </div>
    ...
    return (
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-slate-300">How accountants formally record every business transaction.</p>
        </div>
        ...
        <div className="rounded-xl bg-white/5 border border-white/10 p-5 mb-6">
          ...
          <p className="text-xs text-slate-300 mt-2 text-center">Owner invests $1,000 cash → Cash (Asset ⬆️ Debit), Common Stock (Equity ⬆️ Credit)</p>
        </div>
